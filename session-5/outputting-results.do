global path "$google/Programming Course/Day 6"

/*
Today we are going to focus on some estimation using Stata and the various ways in which you
can output your results for use in other contexts. First, we will build a dataset and then use
that dataset to estimate a model.
*/
...
use "$path/full_data", clear // Notice that you don't need to add the suffix ".dta" because Stata assumes that.
...
/*
The command that I like to use best for creating formatted
tables is "estout." 
...
Within this command, there is a ton of flexibility for getting your
...
// What if you want to do some formatting?

// stat(labe(x)) where "stat" is the statistic name and "x" is the new label

esttab ., cells("count(label(Observations)) mean(label(Mean)) sd(label(Std. Dev.)) p25(label(Q1)) p50(label(Median)) p75(label(Q3))") noobs

// There are too many decimal places. How do we fix that?

// stat(fmt(y)) where "y" is the format
...
/*
What if you don't like the format of the variable names? How do you fix that?

The easiest way to deal with this issue is to employ some variable labels.
...
/*
Honestly, I'm not sure why you really want a correlation table, but I suppose it's a useful
exercise for purposes of replication that you might undertake.

There was no single good way to create a correlation table in Stata that includes a Pearson correlation
below the diagonal and a Spearman correlation above the diagonal. So I wrote my own Stata function to
do just that. The .ado file in in your folder. Move it to a folder on you ADOPATH.
...
To use this command, you simply invoke "makecorr" and feed it a variable list. You can add a title as an option
and choose what output format you would like. 
...
/*
Suppose we want to include year fixed effects in our model (indicators for each year in the sample
minus an excluded base year). How would we accompish this?

One was to do this explicitly in a regression is to use factor variables. You can make a variable
an expanded factor variable by place "i." as a prefix when you specificy the "varlist" in the 
estimation command.
*/
...
/*
If you want to include a high-dimensional set of fixed effects in a regression, you
will want to use a different command from "regress." Two options include xtreg and
reghdfe. If you want to use the latter, you will need to type the following first:
...
//esttab nofe yesfe firmfe using "$path/rating_model", indicate("Year Fixed Effects=*.y") replace rtf nocon stats(firmfe N r2_a , fmt(0 %10.0fc 3 )  label("Firm Fixed Effects" "Observations" "R-squared")) b(4) nonote label title("Determinants of firm-level credit ratings")
...
/*
You can visualize your data in Stata very flexibly. We will go through a basic exercise of trying to graph some
cumulative abnormal returns around earnings announcements based on the sign of the earnings surprise.
*/
