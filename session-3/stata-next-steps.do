global path "/Volumes/HD1/Google Drive/Programming Course/Day 4"


/*
Sorting and "n" subscripting

Stata allows you to conduct operations based on the relative position of an observation
within what is called a "by group." Suppose we wanted to download some Compustat data
but only wanted to keep quarters where there was a single observation for a given company
in a given fiscal quarter that was associated with a single earnings announcement date.
...
/*
Duplicates in terms of gvkey datadate
...
/*
_N refers to the total count of observations in a given "by group." These groups are defined before the
colon and after the command "bysort" which can be abbreviated to "bys" if you want.
...
/*
Dates in Stata are relatively easier to work with. If a variable is a date, we can format it so that
when we browse the data, it is in a visually appealing format. The basic format is %td.
*/
...
/*
Perhaps the most important date related function is that of creating a business calendar so we can track
market trading days and not mess up event time around a particular date.
...
cd "/Volumes/HD1/Google Drive/Programming Course/Day 4"
bcal create trading, replace from(date) dateformat(dmy) center(01jan1960) maxgap(15)


/*
Let's use this business calendar to generate the (-1,+1) window around some earnings announcements for calculating 
...
. count if missing(rdq_b)
  ...
  Apparently 5,055 earnings announcement dates in the dataset fell outside of actual 
  trading days.
*/
...
/*
Notice the "_n" subcripting that references the relative position of an observation
within a "by group."
*/
...
/*
If we want to merge this data with CRSP stock returns, we will have to convert
the business dates back into regular calendar dates
*/
...
Somtimes you will want to create a lead or lag variable and want to ensure that the variable 
is exactly n periods behind or ahead of the current observation.
...
When you obtain data from a service like Thomson One or DataStream, it may be in a format
that is not conducive to analysis without a little bit of help. We will demonstrate this idea
on the Fama-French size and book-to-market returns that we used last week when discussing 
...
Stata has the ability house multiple datasets in memory at once for quick retrieval and joining.
The name of the structure is a data frame. You first need to initialize the frame using either:
...
/*
What about the COMP-CRSP link? Unfortunately, we cannot conduct a many-to-many join of frames in Stata.
Fortunately, we can save the file to our folder and then use the "joinby" command
*/
...
/*
When merging data, Stata provides the result of the merge by observation, which is
stored in the _merge() variable. Here are the types of results
